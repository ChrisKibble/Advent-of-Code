...
$times = $data[0] -split '\s+' | Select-Object -Skip 1
$distances = $data[1] -split '\s+' | Select-Object -Skip 1

$WinCounts = For($run = 0; $run -lt $times.Count; $run++) {
    $thisWin = Get-RecordWinners -TimeMS $times[$run] -RecordDistance $distances[$run]
    $thisWin
}
