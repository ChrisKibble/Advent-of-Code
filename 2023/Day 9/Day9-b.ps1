...
Function Get-PuzzleInput {
    ...
    ForEach($PuzzleLine in $PuzzleInput) {
        ...
        For($index = $PuzzleOutput.Count-2; $index -ge 0; $index--) {
            ...
            $belowNumber = $puzzleOutput[$index+1][0]
            $rightNumber = $puzzleOutput[$index][0]
            [Int]$padNumber = $rightNumber - $belowNumber
            Write-Verbose "    Substracting $belowNumber from $rightNumber = $padNumber"
            $puzzleOutput[$index].Insert(0, $padNumber) | Out-Null
            Write-Verbose "    New Output for index $index is ($($PuzzleOutput[$index] -join ','))"
        }

        $paddedValues += $PuzzleOutput[0][0]
...
$PuzzleOutput = Get-PuzzleInput $x
