...
ForEach($move in $instructions) {
    ...
    $thisZeroCount = [Math]::Floor($notDirection/100)
    ...
    If($dialPosition -lt 0) {
        ...
        If($lastPosition -ne 0) {
            $thisZeroCount++
        }
    } ElseIf($dialPosition -gt 99) {
        $dialPosition = $dialPosition - 100
        $thisZeroCount++
    } ElseIf($dialPosition -eq 0) {
        $thisZeroCount++
    }

    $zeroCount += $thisZeroCount

    Write-Verbose "- The dial is rotated $move to point at $dialPosition; during this rotation, it points at zero $thisZeroCount time(s)."
}
